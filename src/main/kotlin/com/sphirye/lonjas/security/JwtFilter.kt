...
class JwtFilter(private val tokenProvider: TokenProvider) : GenericFilterBean() {
    ...
    override fun doFilter(servletRequest: ServletRequest, servletResponse: ServletResponse, filterChain: FilterChain) {
        ...
        if (StringUtils.hasText(jwt) && tokenProvider.validateToken(jwt)) {
            ...
            Companion.logger.debug("Security Context에 '{}' You have saved your credentials, uri: {}", authentication.name, requestURI)
        } else {
            Companion.logger.debug("No valid JWT token, uri: {}", requestURI)
        }
