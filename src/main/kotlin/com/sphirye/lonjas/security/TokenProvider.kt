...
class TokenProvider(
    ...
    fun validateToken(token: String?): Boolean {
        try {
            ...
            return true
        } catch (e: SecurityException) { logger.info("Invalid JWT signature.")
        } catch (e: MalformedJwtException) { logger.info("Invalid JWT signature.")
        } catch (e: ExpiredJwtException) { logger.info("Expired JWT token.")
        } catch (e: UnsupportedJwtException) { logger.info("Unsupported JWT token.")
        } catch (e: IllegalArgumentException) { logger.info("Invalid JWT token.")
        }
        return false
    }
}